...
ssh越しに`xdg-open`できないかなあとかを調べてたら[socat](http://www.dest-unreach.org/socat/)というツールを知りました。
TCPとかUNIXソケットとかファイルなどの間の双方向の通信をすることができるツールらしいです。
これを使えば標準入出力でやり取りするタイプの言語サーバーの生のやり取りを眺めることができそうだったので、やってみました[^1]。

[^1]: 別にTCP使っててもできると思います。

# やってみる

[Man](http://www.dest-unreach.org/socat/doc/socat.html)の[Examples](http://www.dest-unreach.org/socat/doc/socat.html#EXAMPLES)をみてるとなんとなく使い方がわかります。

今回は標準入出力でやり取りする言語サーバーを使います[^2]。
なので、`socat`では`socat`の標準入出力(`-`)と言語サーバーのプロセス(`EXEC`)の両者を繋ぐようにします。
それで双方向のやり取りを`-r`および`-R`オプションを使ってファイルにダンプします。
例えば`rust-anlyzer`のやり取りをみたければ、`rust-anlyzer`の代わりに

```
socat -r messages_from_client.txt -R messages_from_server.txt - EXEC:rust-anlyzer
```

を実行すれば、`messages_from_server.txt`と`messages_from_client.txt`にJSONRPCの生データが残ります。

[^2]: 自分が試したのは、lua-language-server、rust-anlyzer、jetlsです。

言語サーバークライアントの設定に反映させる必要がありますが、今回はNeovimを使ってやるので、
